(function ($) {
	...
	$('#jabatan').parent().append('<ul class="list-item" id="newmeal_preference" name="meal_preference"></ul>');
	$('#jabatan option').each(function () {
		$('#newmeal_preference').append('<li value="' + $(this).val() + '">' + $(this).text() + '</li>');
	});
	$('#jabatan').remove();
	$('#newmeal_preference').attr('id', 'jabatan');
	$('#jabatan li').first().addClass('init');
	$("#jabatan").on("click", ".init", function () {
		$(this).closest("#jabatan").children('li:not(.init)').toggle();
	});

	var allOptions = $("#jabatan").children('li:not(.init)');
	$("#jabatan").on("click", "li:not(.init)", function () {
		allOptions.removeClass('selected');
		$(this).addClass('selected');
		$("#jabatan").children('.init').html($(this).html());
		allOptions.toggle();
	});
